    ...
    async function fetchSorted(){
        sort_flag = sort_flag==1?0:1;

        let Location = document.getElementById("search-item").value;

        loader.style.display = 'block'
        row_tag.innerHTML = '';
        let response = await fetch('https://twitter-trends-api-deepawali.herokuapp.com/trends/'+sort_flag);
    ...
    async function fetchTrends() {
        let Location = document.getElementById("search-item").value;
        location_tag.textContent = Location.charAt(0).toUpperCase() + Location.slice(1);

        loader.style.display = 'block'
        row_tag.innerHTML = '';
        let response = await fetch('https://twitter-trends-api-deepawali.herokuapp.com/trends/'+Location);
        let data = await response.json();
